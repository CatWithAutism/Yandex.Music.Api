...
public class YTrack
{
    ...
    internal static YTrack FromJson(JToken json)
    {
        ...
        var artists = json.SelectToken("artists")?.Select(artist => YArtist.FromJson(artist.ToString())).ToList();
        var albums = json.SelectToken("albums")?.Select(artist => YAlbum.FromJson(artist.ToString())).ToList();
