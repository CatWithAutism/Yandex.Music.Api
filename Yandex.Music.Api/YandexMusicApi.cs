...
using Yandex.Music.Api.Common.Enums;
...
public class YandexMusicApi : IYandexMusicApi
{
    ...
    private readonly YAuthStorage _storage;
    ...
    public async Task<YAuthorizeResponse> AuthorizeAsync(string login = null, string password = null,
        bool saveCache = false)
    {
        if (saveCache && _storage != null)
        {
        ...
        try
        {
            ...
            if (response.ResponseUri.ToString() == Links.YANDEX_PASSPORT_AUTH)
        ...
        if (saveCache && _storage != null) await _storage.SaveAsync(User);
    ...
    public async Task<YAlbumResponse> GetAlbumAsync(string albumId)
    {
        ...
        using (var response = (HttpWebResponse) await request.GetResponseAsync())
        {
            ...
            album = YAlbumResponse.FromJson(data.ToString());
    ...
    public async Task<YStorageDownloadFileResponse> GetDownloadFilInfoAsync(YTrackDownloadInfoResponse metadataInfo,
        long time)
    {
    ...
    public async Task<YPlaylistChangeResponse> CreatePlaylistAsync(string name)
    {
        ...
        {
            var request =
                new YPlaylistChangeRequest(_httpContext).Create(name, User.Sign, User.Experiments, User.Uid,
                    User.Login);
            var result = string.Empty;
...
//        var auth = GetUserAuthDetails(accounts.DefaultUID);
            ...
            var request =
                new YPlaylistRemoveRequest(_httpContext).Create(kind, User.Sign, User.Experiments, User.Uid,
                    User.Login);
...
//        request.Headers.Add(":scheme", "https");
    ...
    {
        var request = new YNotRecommendTrackRequest(_httpContext).Create(trackKey, _httpContext.GetTimeInterval(),
            User.Sign, User.Uid, User.Login);
    ...
    public async Task<YInsertTrackToPlaylistResponse> InsertTrackToPlaylistAsync(string trackId, string albumId,
        string playlistKind)
    {
        var request = new YInsertTrackToPlaylistRequest(_httpContext).Create(User.Uid, trackId, albumId,
            playlistKind, User.Lang, User.Sign, User.Uid, User.Login, User.Experiments);
...
//      var changeLikeResponse = ChangeLikedTrack(trackKey, value);
    ...
    public async Task<YDeleteTrackFromPlaylistResponse> DeleteTrackFromPlaylistAsync(int from, int to, int revision,
        string playlistKind)
    {
        var request = new YDeleteTrackFromPlaylistRequest(_httpContext).Create(User.Uid, from, to, revision,
            playlistKind,
...
//      var changeLikeResponse = ChangeLikedTrack(trackKey, value);
    ...
    public YDeleteTrackFromPlaylistResponse DeleteTrackFromPlaylist(int from, int to, int revision,
        string playlistKind)
    {
    ...
    {
        var request = new YUnDislikeTrackRequest(_httpContext).Create(trackKey, _httpContext.GetTimeInterval(),
            User.Sign, User.Uid, User.Login);
    ...
    protected string BuildLinkForDownloadTrack(YTrackDownloadInfoResponse mainDownloadResponse,
...
//    }
    ...
    {
        var request = new YSetLikedTrackRequest(_httpContext).Create(value, trackKey,
            _httpContext.GetTimeInterval(), User.Sign, User.Uid, User.Login);
...
//      var changeLikeResponse = ChangeLikedTrack(trackKey, value);
    ...
    {
        var request = new YAddLikedTrackRequest(_httpContext).Create(value, trackKey,
            _httpContext.GetTimeInterval(), User.Sign, User.Uid, User.Login);
