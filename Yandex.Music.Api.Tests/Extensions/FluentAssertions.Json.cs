...
namespace Yandex.Music.Api.Tests.Extensions
{
    ...
    {
        public static void ContainsJson(this GenericCollectionAssertions<JToken> asserions,
        ...
        private static void CheckJsonFile(string key, JToken value, JToken assertValue, Type valueType)
        {
            ...
            {
                if (assertValue == null) Assert.False(true, $"Field[{key}] - not found field");
                ...
                foreach (var fileFieldDictionary in fileDictionartValue)
                {
                    ...
                    if (isArray)
                    {
                        if (!fileFieldValue.Equals("*no_valid_field*"))
            ...
            {
                var arrayFileProperty = (JArray) value;
                var arrayAssertProperty = (JArray) assertValue;

                for (var i = 0; i < arrayAssertProperty.Count; i++)
                {
        ...
        private static bool CheckDateTime(string key, JToken value, JToken assertValue)
        {
            ...
            var isDateTime = DateTimeOffset.TryParse(value.ToString(), out dateValue);
            ...
            var errorMessage = $"The field is not equivalent\nField [{key}]: {assertValue} is not equal to {value}";
